...
MOD_ID = 'ProjectALifeNPCs_PTBR_Hugo'
OUT = ROOT / 'build'
...
[b]Requer o mod original.[/b] Ative os dois; esta tradução carrega depois do original. Mod ID da tradução: ProjectALifeNPCs_PTBR_Hugo. Feita para a versão {upstream_version} do original.
...
def write_speech_layer():
    ...
    speech_dir = lua_root / 'shared/ALifePTBRHugo/Speech'
    ...
    chat_file = ROOT / 'translations/chat/intents_ptbr.py'
    chat = []
    if chat_file.exists():
        spec = importlib.util.spec_from_file_location('intents_ptbr', chat_file)
    ...
    body += ['    { %s, %s, %s },' % tuple(lua_literal(chr(34), v) for v in (e['id'], '|'.join(e['patterns']), e.get('keywords', '')))
             for e in chat]
    body += ['}', '']
    write_lua(lua_root / 'shared/ALifePTBRHugo/ChatIntents.lua', '\n'.join(body))
    return len(items)
